 ...
 * Sections: State | Data | DOM | Filters | Table | Custom | PDF | Modal | Utils
 */
...
const State = {
  ...
  filteredQuestions: [],
  selectedIds:      new Set(),
  currentMode:      'pyq',  // 'pyq' | 'custom'
  customParsed:     [],     // parsed custom questions
...
/* ============================================================
   SECTION 5: TABLE — Render & Selection
   ============================================================ */
const Table = (() => {
  ...
  function render() {
    ...
    if (State.filteredQuestions.length === 0) {
      tbody.innerHTML = '<tr class="empty-row"><td colspan="6">No questions match your filters. Try adjusting or resetting them.</td></tr>';
      syncSelectAll();
      return;
    }
    ...
    State.filteredQuestions.forEach(q => {
      fragment.appendChild(buildRow(q));
    });
  ...
  function updateSelectedBar() {
    ...
    bar.style.opacity = count > 0 ? '1' : '0.7';
  }
...
/* ============================================================
   SECTION 6: CUSTOM QUESTION PARSER
   ============================================================ */
const CustomParser = (() => {

  // Patterns to match:
  // Q1. text [15]  |  Q1. text (15)  |  1. text 15 marks  |  Q1 text [15]
  const PATTERN = /^(?:Q\s*)?(\d+)[.)]\s*(.+?)\s*[\[(](\d+)[\])]\.?\s*$|^(?:Q\s*)?(\d+)[.)]\s*(.+?)\s+(\d+)\s*(?:marks?)?\.?\s*$/i;
...
const PDFGenerator = (() => {
  ...
  // Layout constants (mm)
  const MARGIN_LEFT_OUTER = 10;   // space before Q number
  const MARGIN_LEFT_LINE  = 22;   // vertical line x position
  const MARGIN_LEFT_TEXT  = 24;   // text area start
  const MARGIN_RIGHT_LINE = 186;  // right margin line x
  const MARGIN_RIGHT_TEXT = 184;  // text boundary
  const MARGIN_TOP        = 20;   // top of content
  const MARGIN_BOTTOM     = 18;   // space for footer
  const RIGHT_NOTE_X      = 188;  // "Candidates must not..." text x
  const RIGHT_NOTE_W      = 20;   // width of right margin note area

  // Pages per question by marks
  ...
  // Draw page borders & margins (left line, right line)
  ...
  // Draw right-margin note "Candidates must not write on this margin"
  function drawRightMarginNote(doc) {
    ...
    noteLines.forEach(line => {
      doc.text(line, RIGHT_NOTE_X + 1, y);
      y += 4;
    });
  ...
  // Draw footer: booklet code (left), page number (center)
  ...
  // Draw a question page
  function drawQuestionPage(doc, question, qSeqNum, marksText, footerCode, pageNum) {
    drawPageFrame(doc);
    drawRightMarginNote(doc);

    const textAreaWidth = MARGIN_RIGHT_LINE - MARGIN_LEFT_TEXT - 2;

    // Question number (outside margin line)
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text(`Q${qSeqNum}.`, MARGIN_LEFT_OUTER, MARGIN_TOP + 5);

    // Question text (inside area)
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10.5);
    const lines = wrapText(doc, question, textAreaWidth - 20, 10.5);

    let y = MARGIN_TOP + 5;
    lines.forEach(line => {
      doc.text(line, MARGIN_LEFT_TEXT, y);
      y += 6;
    });

    // Marks — same vertical level as first line of question, right-aligned
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
  ...
  function generate(questions, footerCode, titleNote) {
    ...
    questions.forEach((q, index) => {
      const qSeqNum  = index + 1;
      const marks    = q.marks;
      const marksStr = `${marks}`;
      const totalPgs = pagesForMarks(marks);
      ...
      drawQuestionPage(doc, q.question, qSeqNum, marksStr, code, pdfPageNumber);
      pdfPageNumber++;
...
const Modal = (() => {
  ...
  function confirm() {
    ...
    close();
    PDFGenerator.generate(_pendingQuestions, code, title);
  }
...
/* ============================================================
   SECTION 9: CUSTOM PANEL CONTROLLER
   ============================================================ */
const CustomPanel = (() => {
  ...
  function updateCounter() {
    const raw   = DOM.customInput().value;
    const count = CustomParser.countDetected(raw);
  ...
  function handlePreview() {
    ...
    if (parsed.length === 0) {
      showError('No valid questions found. Please check the format (e.g. "Q1. Your question. [15]").');
      return;
    }
...
const EventBinder = (() => {
  ...
  function bindAll() {
    ...
    DOM.selectAll().addEventListener('change', e => {
      Table.handleSelectAll(e.target.checked);
    });
...
async function init() {
  try {
    ...
    Utils.showToast('Application failed to initialise.', 'error');
  }
}
